...
public class InfernoUtilsCommand {
    ...
    public static int combatAdd(CommandContext<CommandSourceStack> context) {
        ...
        InfernoUtils.playerCombatTracker.put(player.getStringUUID(), currentTime);

        List<UUID> forRemoval = new ArrayList<>();
        InfernoUtils.combatBossEvents.forEach(((uuid, serverBossEvent) -> {
            serverBossEvent.removePlayer(player);
            if (serverBossEvent.getPlayers().isEmpty())
                forRemoval.add(uuid);
        }));
        forRemoval.forEach(uuid -> InfernoUtils.combatBossEvents.remove(uuid));
        ...
        InfernoUtils.combatBossEvents.put(combatBossEvent.getId(), combatBossEvent);

        combatBossEvent.addPlayer(player);
        InfernoUtils.playerCombatBossEvents.put(player.getUUID(), combatBossEvent.getId());
